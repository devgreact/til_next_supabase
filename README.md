...
- posts > create Policy > `Authenticated users can createpost` > `insert` > `authenticated` > `(select auth.uid()) = author_id` > 저장
...
- posts > create Policy > `Autheicated users can delete won poast` > `delete` > `authenticated` > `(select auth.uid()) = author_id` > 저장
