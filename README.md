...
- `/src/components/profile/ProfileInfo.tsx` 배치
...
### 3.1. 프로필 모달의 상태를 관리할 `Store 생성`

- `/src/stores/profileEdiotorModal.ts 파일` 생성
...
import { Button } from '@/components/ui/button';
import useProfileData from '@/hooks/queries/useProfileData';
import { useProfileEditor } from '@/stores/profileEdiotorModal';
...
export function useUpdateProfile(calllback?: UseMutationCallback) {
  return useMutation({
    mutationFn: updateProfile,
    onSuccess: () => {
      if (calllback?.onSuccess) calllback.onSuccess();
    },
    onError: error => {
      if (calllback?.onError) calllback.onError(error);
    },
  });
}
```
